...
export async function generateModelsAsync(config) {
    console.log(Colors.green(`Model generator started \n`));
    const outputDir = config.outputDir ? `${config.outputDir}/`.replaceAll('//', '/') : `./`;
    const contentTypesFolderName = `content-types/`;
    const contentTypeSnippetsFolderName = `content-type-snippets/`;
    const taxonomiesFolderName = `taxonomies/`;
    const projectFolderName = `project/`;
    const contentTypesFolderPath = `${outputDir}${contentTypesFolderName}`;
    ...
    try {
        if (config.sdkType === 'delivery') {
            ...
            const moduleResolution = config.moduleResolution ?? 'node';
            ...
            const types = commonHelper.sortAlphabetically((await managementClient.listContentTypes().toAllPromise()).data.items, (item) => item.name);
            const snippets = commonHelper.sortAlphabetically((await managementClient.listContentTypeSnippets().toAllPromise()).data.items, (item) => item.name);
            const taxonomies = commonHelper.sortAlphabetically((await managementClient.listTaxonomies().toAllPromise()).data.items, (item) => item.name);
            ...
            if (config.exportProjectSettings?.exportAssetFolders || exportAllProjectSettings) {
                ...
                console.log(`Found '${Colors.yellow(projectGenerator.getAssetFoldersCount(assetFolders).toString())}' asset folders`);
            }
            else {
            ...
            const barrelExportFilename = 'index.ts';
            // content types
            ...
            const contentTypeBarrelExportPath = `${contentTypesFolderPath}${barrelExportFilename}`;
            ...
            const contentTypeSnippetsBarrelExportPath = `${contentTypeSnippetsFolderPath}${barrelExportFilename}`;
            ...
            const taxonomiesBarrelExportPath = `${taxonomiesFolderPath}${barrelExportFilename}`;
            ...
            const projectBarrelExportPath = `${projectFolderPath}${barrelExportFilename}`;
            ...
            const mainBarrelExportPath = `${outputDir}${barrelExportFilename}`;
